    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-dark-secondary overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('stock-entries.update', $stockEntry) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-input id="quantity" class="block mt-1 w-full bg-gray-700 text-gray-200 border-gray-600" type="number" name="quantity" :value="old('quantity', $stockEntry->quantity)" min="1" required />
                                @error('quantity')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="entry_date" class="block mt-1 w-full bg-gray-700 text-gray-200 border-gray-600" type="date" name="entry_date" :value="old('entry_date', $stockEntry->entry_date->format('Y-m-d'))" required />
                                @error('entry_date')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="3" class="block mt-1 w-full border-gray-600 bg-gray-700 text-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $stockEntry->notes) }}</textarea>
                                @error('notes')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
